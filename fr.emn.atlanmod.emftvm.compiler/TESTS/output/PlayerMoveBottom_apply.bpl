...
implementation PlayerMoveBottom_apply (__trace__: ref,s: ref,rec: ref,pac: ref,grid2: ref,grid1: ref,act: ref) returns ()
{
...
assert read($srcHeap, $Unbox(Seq#Index(stk, Seq#Length(stk)-2)), alloc);
assert !isset($acc, $Unbox(Seq#Index(stk, Seq#Length(stk)-2)), pacman$Grid.hasPlayer);
$srcHeap := update($srcHeap, $Unbox(Seq#Index(stk, Seq#Length(stk)-2)), pacman$Grid.hasPlayer, $Unbox(Seq#Index(stk, Seq#Length(stk)-1)));
$acc := set($acc, $Unbox(Seq#Index(stk, Seq#Length(stk)-2)), pacman$Grid.hasPlayer, true);
assume $IsGoodHeap($srcHeap);
stk := Seq#Take(stk, Seq#Length(stk)-2);
...
label_END_rule_PlayerMoveBottom_applier:
