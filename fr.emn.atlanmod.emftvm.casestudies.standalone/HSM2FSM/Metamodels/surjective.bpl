...
is1!=null && read($s, is1, alloc) && dtype(is1) <: HSM$InitialState 
 &&  !((read($s, is1, HSM$AbstractState.compositeState)==null))  &&
...
t1!=null && read($s, t1, alloc) && dtype(t1) <: HSM$Transition 
 ...
 &&  read($s, t1, HSM$Transition.source) == src && read($s, t1, HSM$Transition.target) == trg && read($s, c, HSM$AbstractState.compositeState) == src && !(dtype(trg) == HSM$CompositeState) && !(c == src)  &&
